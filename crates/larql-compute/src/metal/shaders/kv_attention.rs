...
//!
//! Two kernels:
//!   - `kv_attention_fast`: T ≤ 1024, 4KB threadgroup (high occupancy)
//!   - `kv_attention_long`: T > 1024, 32KB threadgroup (KV replay with large context)
//!
//! The dispatcher picks based on T. Both use simd_max/simd_sum for reductions
//! and float4 Q·K dot products.

pub const SHADER: &str = r#"
// Fast decode attention — 4KB threadgroup, high occupancy.
// Used for normal decode (T grows from 1 to ~1024).
kernel void kv_attention_fast(
...
// Long-context decode attention — 32KB threadgroup, lower occupancy.
// Used when T > 1024 (KV replay with large precomputed context).
