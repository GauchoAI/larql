...
/// Same as `apply_rope_partial`, but applies RoPE at absolute positions
/// `pos_offset..pos_offset+seq_len` instead of `0..seq_len`. Used by the
/// single-token KV-cached decode path where the current token sits at
/// position = past_len.
pub fn apply_rope_partial_at_pos(
    x: &Array2<f32>,
    num_heads: usize,
    head_dim: usize,
    rope_base: f64,
    ...
    for pos in 0..seq_len {
        for h in 0..num_heads {
            let offset = h * head_dim;
            for i in 0..half_rotary {
                let theta = (pos + pos_offset) as f64 * inv_freq[i];
                let cos_t = theta.cos() as f32;
                let sin_t = theta.sin() as f32;
                let x0 = x[[pos, offset + i]];
                let x1 = x[[pos, offset + half_rotary + i]];
